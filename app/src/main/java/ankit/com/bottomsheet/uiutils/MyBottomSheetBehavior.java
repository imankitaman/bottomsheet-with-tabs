...
public class MyBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
     ...
     */
    public abstract static class MyBottomSheetCallback {
    ...
    /*
      New Custom Anchor Threshold
     */
    private static final float ANCHOR_THRESHOLD = 0.45f;//0.45 will cover almost 50% of screen
    ...
    private MyBottomSheetCallback mCallback;
    ...
    /**
     * Default constructor for instantiating MyBottomSheetBehavior.
     */
    public MyBottomSheetBehavior() {
    }

    /**
     * Default constructor for inflating MyBottomSheetBehavior from layout.
     *
    ...
    public MyBottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MyBottomSheetBehavior_Params);
        setPeekHeight(
                a.getDimensionPixelSize(R.styleable.MyBottomSheetBehavior_Params_peekHeight, 0));
        setHideable(a.getBoolean(R.styleable.MyBottomSheetBehavior_Params_hideable, false));
        a.recycle();
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        mAnchorOffset = (int) Math.max(mParentHeight * mAnchorThreshold, mMinOffset);// parent.getHeight() * 0.50 -> 50% of parent
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target) {
        ...
        if (mHideable && shouldHide(child, getYVelocity())) {
            ...
            int currentTop = child.getTop();
            if (Math.abs(currentTop - mAnchorOffset) < Math.abs(currentTop - mMinOffset)) {//breakpoint on 50%
                top = mAnchorOffset;
     ...
     */
    public void setAnchorSheetCallback(MyBottomSheetCallback callback) {
        mCallback = callback;
    }
...
// commented for Scroll Fix Inside ViewPager
/*    private View findScrollingChild(View view) {
    ...
    private View findScrollingChild(View view) {
        if (view instanceof NestedScrollingChild) {
            ...
            if (view instanceof ViewPager) {//check for Using ViewPager Fix
                ViewPager viewPager = (ViewPager) view;
                View currentViewPagerChild = ViewPagerUtils.getCurrentView(viewPager);//ViewpagerUtils provide current view from Viewpager
